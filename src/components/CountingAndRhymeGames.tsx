...
export function CountingGame({ difficulty }: { difficulty: Difficulty }) {
  ...
  const obj = useRef(OBJECTS[Math.floor(Math.random() * OBJECTS.length)]);
  const count = useRef(randomCount(difficulty));
  const options = useRef(makeOptions(count.current));
  ...
  function pick(n: number) {
    ...
    setPicked(n);
    const correct = n === count.current;
    if (correct) { fire(); addStars(difficulty === "easy" ? 1 : difficulty === "medium" ? 2 : 3); setScore((s) => s + 1); speak(`Yes! ${count.current}!`); }
    else { speak(`The answer is ${count.current}`); }
  }

  function next() {
    obj.current = OBJECTS[Math.floor(Math.random() * OBJECTS.length)];
    count.current = randomCount(difficulty);
    options.current = makeOptions(count.current);
    setPicked(null);
    setRound((r) => r + 1);
  ...
  return (
    <div className="card-soft mx-auto max-w-2xl p-6 text-center">
      ...
      <div className="mb-4 flex justify-center">
        <Mascot mood={picked === null ? "thinking" : picked === count.current ? "cheer" : "sad"} />
      </div>
      <p className="mb-3 text-lg font-bold">How many do you see?</p>
      <div key={round} className="mb-6 flex flex-wrap justify-center gap-1 rounded-2xl bg-muted/30 p-4">
        {Array.from({ length: count.current }).map((_, i) => (
          <span key={i} className="text-4xl">{obj.current}</span>
        ))}
      </div>
      <div className="grid grid-cols-4 gap-3">
        {options.current.map((n) => {
          const isCorrect = n === count.current;
          const isPicked = n === picked;
          return (
            <button key={n} onClick={() => pick(n)} className={`card-soft rounded-2xl py-4 font-display text-3xl font-bold transition-all ${
              picked ? isCorrect ? "bg-mint scale-110 shadow-md" : isPicked ? "bg-destructive/30" : "opacity-50" : "bg-sky hover:scale-105"
            }`}>{n}</button>
          );
        })}
      </div>
...
export function RhymeTimeGame({ difficulty }: { difficulty: Difficulty }) {
  const pool = difficulty === "easy" ? RHYMES.slice(0, 4) : RHYMES;
  const [score, setScore] = useState(0);
  const [round, setRound] = useState(0);
  const [picked, setPicked] = useState<string | null>(null);
  const entry = useRef(pool[Math.floor(Math.random() * pool.length)]);
  const options = useRef(makeRhymeOptions(entry.current));
  const { active: confetti, fire } = useConfetti();

  // Say the full word, not letter by letter
  function sayWord() {
    speak(entry.current.word.toLowerCase());
    setTimeout(() => speak(`Which word rhymes with ${entry.current.word.toLowerCase()}?`), 800);
  }
  ...
  function pick(word: string, correct: boolean) {
    ...
    if (correct) {
      ...
      setScore((s) => s + 1);
      speak(`Yes! ${entry.current.word.toLowerCase()} and ${word.toLowerCase()} rhyme!`);
    } else {
  ...
  function next() {
    ...
    entry.current = e;
    options.current = makeRhymeOptions(e);
    setPicked(null);
    setRound((r) => r + 1);
  ...
  return (
    <div className="card-soft mx-auto max-w-2xl p-6 text-center">
      ...
      <div className="mb-4 flex justify-center">
        <Mascot mood={picked === null ? "idle" : options.current.find((o) => o.word === picked)?.correct ? "cheer" : "sad"} />
      </div>
      <p className="mb-2 text-sm font-semibold text-muted-foreground">Which word rhymes with:</p>
      <button onClick={sayWord} className="mb-6 inline-flex items-center gap-3 rounded-2xl bg-pink px-6 py-4 font-display text-3xl font-bold shadow-md hover:scale-105 transition">
        {entry.current.emoji} {entry.current.word} 🔊
      </button>
          ...
          return (
            <button key={opt.word} onClick={() => pick(opt.word, opt.correct)} disabled={!!picked}
              className={`card-soft rounded-2xl py-5 font-display text-2xl font-bold transition-all ${
                picked ? opt.correct ? "bg-mint scale-105 shadow-md" : isPicked ? "bg-destructive/30" : "opacity-50" : "bg-butter hover:scale-105"
              }`}>
              {opt.word}
            </button>
          );
